...
public class ScoketController {
	...
	@RequestMapping(value = "/socketRequest1", produces = "application/json; charset=utf-8")
	@ResponseBody
	public JsonReturn socketRequest1(HttpServletRequest request) {
		// 返回结果
		JsonReturn jsonReturn = new JsonReturn();
		...
		try {
			...
			// 组装业务数据，发送给客户端
			// 根据不同的端口，写入对应的socket客户端
			socketServerService.outPutSocketToClient1(port, sb);
			Thread.sleep(10L);
			// 抓取客户端返回的结果
			SocketBean resultScoket = socketServerService.inPutSocketFromClient1(InitUtil.hospitalIdPortMap.get(hId),
					sb.getUid());
			if (resultScoket != null) {
				Integer code = resultScoket.getCode();
				jsonReturn.setRet(code);
	...
	@RequestMapping(value = "/socketRequest2", produces = "application/json; charset=utf-8")
	@ResponseBody
	public JsonReturn socketRequest2(HttpServletRequest request) {
		// 返回结果
		JsonReturn jsonReturn = new JsonReturn();
		...
		try {
			...
			// 组装业务数据，发送给客户端
			// 根据不同的端口，写入对应的socket客户端
			socketServerService.outPutSocketToClient2(port, sb);
			Thread.sleep(10L);
			// 抓取客户端返回的结果
			SocketBean resultScoket = socketServerService.inPutSocketFromClient2(InitUtil.hospitalIdPortMap.get(hId),
					sb.getUid());
			if (resultScoket != null) {
				Integer code = resultScoket.getCode();
				jsonReturn.setRet(code);
